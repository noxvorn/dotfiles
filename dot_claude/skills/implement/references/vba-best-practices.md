# VBA `.bas` / `.cls` ベストプラクティス

Excel VBA の exported standard module (`.bas`) / class module (`.cls`) を作成・編集する時の実装指針。
既存挙動や Excel / VBE の互換性を優先すべき場面では、標準から外す理由と影響範囲を明示する。
文字コード、metadata、識別子、`Option Explicit`、公開面の制約は [rules/vba.md](../../../rules/vba.md) を正本とする。
