...
1. **複数案を実比較した**: 仮想の比較ではなく、実在する代替案を検討した。
2. **覆すコストが高い**: 公開 IF / データ形式 / 永続化 / 認証 / 依存などに関わり、後で方向転換すると痛い。
3. **捨てた案に再検討の価値がある**: 将来の読み手が「なぜ A でなく B か」を疑問に思う。

戻しやすい、明らか、実質的な代替案がない判断では ADR を作らない。
...
[実比較した案と、捨てた理由。3 条件のうち「再検討の価値」を満たす時だけ書く。]
