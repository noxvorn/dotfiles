...
- 要件や成功条件の整理が主目的なら plan-product を優先する。
- 技術的な実装順序や影響範囲の整理が主目的なら plan-architect を優先し、挙動維持の実施段階で本スキルを使う。
...
- 変更対象の近傍にある既存実装パターンを確認する。
- 不明点は最小限の質問で補う。

## 削除系リファクタの見方

- 未使用コード削除、重複統合、依存関係整理は、確認しやすいものから小さく進める。
- 発見事項は `SAFE` / `CAREFUL` / `RISKY` で軽く分類する。
- `SAFE`: 未使用 import や内部限定の未使用要素など、静的に追いやすいもの。
- `CAREFUL`: 動的参照、設定値、文字列参照経由で使われる可能性があるもの。
- `RISKY`: 公開インターフェース、共有ユーティリティ、運用前提に近いもの。
- 迷う要素は削除候補として即断せず、確認項目として切り出す。
...
- 既存テスト、build、lint、代替確認のどれで回帰を見るかを先に決める。
...
- 参照検索で、静的な参照が残っていないかを確認する。
- 動的参照や文字列参照の可能性がないかを確認する。
- 公開インターフェースや共有境界に含まれないかを確認する。
- 確認材料が弱い場合は削除を急がず、追加調査や保留を選ぶ。
...
- リファクタと挙動変更を同時に行う。
- 検証なしで複数ステップを進める。
- 既存の局所パターンを無視して、新しい設計流儀へ寄せる。
- 数値目標や見た目のきれいさだけを理由に分割する。
- 判断材料が弱いまま「未使用らしい」ものを勢いで削除する。
...
- 近傍コードの命名・責務・エラー処理に寄せて違和感を減らす。
- 重複統合では、既存文脈に最も合い、責務が明確で、利用実績や検証実績のある実装を残す。
...
- 削除系リファクタで迷う場合は [references/refactor-cleanup-heuristics.md](references/refactor-cleanup-heuristics.md) を参照する。
