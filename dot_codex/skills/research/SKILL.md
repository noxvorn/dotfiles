---
name: research
description: 「まず調べたい」「バグの原因を切り分けたい」「外部変化や品質・セキュリティ・保守性への影響を事実ベースで整理したい」といった調査依頼で使う。初見 repo の grounding を含め、facts、unknowns、options、recommendation、next_step を整理する。要件整理したい時は `product-planning` スキル、architecture 改善候補を探したい時は `improve-codebase-architecture` スキル、実装前 scope を決めたい時は `implementation-planning` スキルを使う。
metadata:
...
architecture 改善候補の提示や候補選択後の grilling が主目的の場合は、`improve-codebase-architecture` に切り替える。
...
- 調査結果を前提にした実装
- 実装前 scope の整理
- architecture 改善候補の探索と grilling
...
- architecture 改善候補を出す段階に入ったら、`improve-codebase-architecture` スキルへ進める。
