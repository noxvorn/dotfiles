---
name: security-scan
description: 「セキュリティリスクを下げたい」「hardening の対象と検証観点を決めたい」といった security 案件で使う。対象リスク、今回の security scope、必要な hardening、重点検証項目を整理する。外部変化への追従が主題なら `compat-assessment`、通常 bugfix が主題なら `bug-diagnosis` を優先する。
metadata:
