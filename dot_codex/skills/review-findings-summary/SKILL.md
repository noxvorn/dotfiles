...
description: レビュー出口整理手順。`code-review` や reviewer 結果を findings-first で整理し、未検証事項と残リスクを分ける。
metadata:
...
レビュー対象の確定と reviewer 起動は `code-review` が担い、このスキルは findings-first の整理に専念する。
...
- `code-review` から渡された reviewer agent 結果を出口整理する場面。
...
- 対象差分または対象範囲を確認する。
- 実施済み検証と未検証事項を分ける。
- `code-review` から渡された場合も、ここでは reviewer agent の追加起動より出口整理を優先する。
