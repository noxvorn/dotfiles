...
1. `可読性優先`: 名前、責務、構造、コメントは、あとから読む人が意図を追いやすいことを優先する。
2. `KISS`: 読みやすさを満たしたうえで、理解しやすく検証しやすい最小案を選ぶ。
3. `YAGNI`: 将来のためだけの抽象化、設定、拡張ポイントを先回りで入れない。
4. `DRY`: 実害のある重複だけを解消し、上位原則を損なう共通化なら重複を残す。
