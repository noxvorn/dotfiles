...
- 優先順位の最終判断は「原則（優先順）」に従う。
- 既存の標準テスト・ビルド手順があれば実行し、できない場合は理由を報告する。
- トレードオフ記録は原則 4〜7 の衝突時に限り、コードコメントで理由を最小限に記録する。
...
- 原則 4〜7 間のトレードオフが予想される箇所を事前に列挙する。

### 3) 実装中のチェック

- 原則に照らして実装し、原則 4〜7 で逸脱がある場合は理由を残す。
...
番号が小さいほど優先する。
原則が衝突する場合は優先順位に従う。
原則 1〜3 は必須とし、例外を認めない。
最終優先順位は以下とする。
1. 本スキルの原則 1〜3
2. 既存の README・CONTRIBUTING・lint・formatter・CI 設定
3. 本スキルの原則 4〜7

### 1. No Overengineering

- 要件達成に不要な抽象化・拡張ポイント・設計層を追加しない
- 将来の可能性だけを理由に構造を複雑化しない
- 実装・運用・レビューの負荷を増やす設計は採用しない
- 必要性が実証されるまでは最小実装を維持する
