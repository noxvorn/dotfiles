...
- 対象は Excel VBA の exported `.bas` / `.cls` ファイル。
- `.bas` は standard module、`.cls` は class module として扱う。
- VB6、Access VBA、`.frm`、workbook 本体の生成・変換は v1 対象外。
...
- 標準モジュール名とクラス名、つまり `Attribute VB_Name` は 31 文字以内にする。
- `Attribute VB_Name` は先頭を英字にし、空白、ピリオド、`!`、`@`、`&`、`$`、`#` を含めない。予約語や同一 project 内の重複名も避ける。
- ファイル名の拡張子なし部分が有効な VBA module 名でない場合は、黙って別名を作らず、rename / 正規化方針を確認する。やむを得ずファイル名と `VB_Name` がずれる場合は理由を報告する。
- プロシージャ名、関数名、変数名、引数名、定数名など一般の識別子は 255 文字以内にし、VBA naming rules に従う。
- 既存ファイルでは、明示的な rename / 正規化依頼がない限り、既存の `Attribute` 行と module metadata を保持する。
- 既存ファイルが別 encoding / 改行の場合は、文字化けや差分肥大を避けるため実態を確認し、変換するなら理由を明示する。
...
- public members、default member、`VB_PredeclaredId`、`VB_Exposed` など呼び出し側や生成方法に影響する属性変更は公開面として扱い、変更前に参照と影響範囲を確認する。
...
- workbook、worksheet、range、table などの境界は変数に束縛し、処理対象を読み手が追える形にする。
...
- エラーを握りつぶさない。呼び出し側へ返す、再送出する、または利用者に説明できる形で扱う。
