---
name: planning
description: 文脈上、実装、機能追加、リファクタ、設定変更、設計変更、振る舞い変更に入る前に自動使用する。ユーザー意図、目的、成功条件、要件、制約、設計、実装順序、確認方法を一問ずつ確認し、合意済みの計画、成果物、durable knowledge へ整理する。バグ原因や外部変化の事実調査は `research`、差分作成は `implementation` スキルを使う。
metadata:
...
- draft を出したら、スコープ、矛盾、曖昧語、未確認事項、確認方法を self-review し、必要なら修正する。
- 実装へ進む前に、未確認事項と残リスクを明示する。
...
出力 key は `open_questions` を使い、成果物本文の見出しは `未確認事項` を使う。
