...
- スコープはデフォルトでは付けず、まず scope なしで自然に伝わる案を優先する。
- リポジトリ規約や識別上の必要性が明確な場合にのみスコープを付ける。
- 対象パスや変更領域が見えても、それだけを理由に機械的にスコープを付けない。
...
- `git-commit` スキルから補助的に呼ばれる場合は、その文脈を優先して受け取る。
...
- まず scope なしの header を作り、それで意味が十分に通るなら scope は付けない。
- 付ける場合は「付けないと曖昧になる理由」を説明できることを条件にする。
...
- scope を付けるときは「なぜ scope なしでは弱いのか」を自分で説明できるか確認する。
