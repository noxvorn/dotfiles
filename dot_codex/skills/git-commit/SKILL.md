...
- 1コミット1目的を原則とする。
- 1コミット1説明を原則とし、コミットタイトル案が2つ以上必要なら最初から別コミットに分ける。
- 1コミット1可逆性を原則とし、片方だけを取り消したくなる可能性がある変更は最初から別コミットに分ける。
- レビューやテストを独立して行える変更は、同じ作業文脈でも別コミットに分ける。
- 大きな削除・移行・構成変更は、通常の改善変更と分離する。
- 未プッシュで混在コミットに気づいた場合は、分割してから履歴を確定する。

## 目的混在・粒度の判定ルール

- 変更ファイルと hunk を「何のための変更か」でグルーピングし、2グループ以上なら混在と判定する。
- 同一コミットに含めてよいのは、同じ目的に直接必要で、単一のコミットタイトルで自然に説明できる変更のみとする。
- 同じ目的でも、機能Aと機能B、修正Aと修正Bのように利用者価値や修正内容が独立している場合は別コミットに分ける。
- 同じ目的でも、将来の準備リファクタと本体変更のように段階的にレビューできる場合は別コミットに分ける。
- 迷う場合は混在扱いにし、先にコミット候補を複数提示してから分割を提案する。
- 例:
  - まとめてよい: 機能追加とそれに必要なテスト更新。
  - まとめてよい: 単純なリネームと、それに追従する参照更新。
  - 分ける: 機能Aと機能B、修正Aと修正B、機能追加と将来の準備リファクタ、ドキュメント更新と設定変更、リファクタと依存更新、機能追加と別件の修正。
...
- 変更を目的単位で分類し、混在している場合や同じ目的でも粒度が粗い場合はコミットを停止して分割方針を合意する。
- 未追跡ファイルや大きな生成物、バイナリが含まれる場合は対象を確認し、必要なら除外を提案する。
- 合意前に `git add -A` やコミットを実行しない。

### 3.5) コミット計画

- 差分確認後、コミット候補を1件ずつ列挙する。
- 各候補に「タイトル案」「含めるファイル/hunk」「単独で成立する理由」を付ける。
- 候補が複数ある場合は、1件ずつ順に 4) から 8) を繰り返す前提で合意を取る。
- 候補を1件に絞れない場合やタイトル案が複数必要な場合は、さらに分割を検討する。

### 4) ステージ

- 指定がなければ、合意した1件のコミット候補の範囲のみをステージする。
- 指定があれば `git add <paths>` などでその範囲のみステージする。
- 1ファイル内に複数目的の差分や複数コミット候補が混在する場合は、`git add -p` で対象に関係する hunk のみをステージする。
