...
確立済み英語用語を残す方が明確な場合を除き、本文は日本語で書く。
...
- prose は日本語。翻訳すると精度が落ちる確立済み英語 domain terms は残す。
- 定義は最大 1 文。実装 behavior は書かない。
...
- secrets、credentials、private config values、未公開個人情報、作業メモ、spec、実装判断を入れない。
...
- 各 context、対応する `CONTEXT.md`、context 間の関係を書く。
