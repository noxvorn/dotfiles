---
name: scribe
description: 実装が固まった後の README / docs（仕様・使い方）、ADR（なぜそうしたか）、notes（背景・調査メモ）を作成・更新する時に使う。開発駆動の事前 doc は対象外で、third-party 保守に必要な後追い doc に絞る。
metadata:
...
- **仕様・使い方 doc**: 実装が固まった後、third-party が使い方を理解する必要がある時。
- **ADR**: 実装中の分岐点で、下記 3 条件を満たす時。
- **notes**: 背景・調査メモを残す価値がある時（任意）。

## doc 要否（silent skip 禁止）

実装一段落時と commit 前に「仕様 doc 追従の要否」を一言明示する。黙って飛ばさない。

- **要**: 公開 IF / CLI / API の追加・変更、新機能・新しい使い方、設定項目の追加・変更、third-party が挙動を知る必要がある変更。
