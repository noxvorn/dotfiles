...
- `capture-knowledge` の ADR 作成、採用、supersede action で採用判断契約を一致させる。
...
- `capture-knowledge` は旧 ADR 更新時に新 ADR 側の関係行を補完せず、明示された関係だけを検証する。

## Direct ADR flows

- `capture-knowledge` は新 ADR をまず `Proposed` として作る。
