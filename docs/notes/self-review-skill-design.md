...
**過剰さは「読み手が理解・判断するのに必要な水準を超えている部分」。** 絶対評価ではなく、目的に対する相対評価にしてある。判断に効かない情報は、正しくても過剰。
...
2026-09-01 に `coding-standards.md` へ打ち切り順を入れた時、この型が 3 つ出た。段 4 が「`<input type="date">` で足りる」と「client 側の機能は検証に数えない」を並べていて段の判定が決まらない、security primitive の carve-out を段リストの後ろに置いたので段 3 で止まった読み手に届かない、reviewer の指摘へ対応した文言の範囲が指摘の趣旨より狭く同じ穴が別の段に残る。いずれも文としては正しく、他の観点には引っかからない。使う場面を再現して初めて出た。
...
直すと別の問題が生まれる、という観察自体は正しい。数値を消せば指示語が指す先を失い、記述を強めれば根拠を超え、短くすれば参照が空振りする。だから手順に「直したら、その直しに関係する項目だけ再確認する」を残してある。全部やり直さないのは、通し切った項目をもう一度通しても、直しが触っていない箇所からは新しいものが出ないため。
