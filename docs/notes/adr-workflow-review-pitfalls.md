...
- `grill-with-docs` の ADR 作成、採用、supersede 更新で採用判断契約を一致させる。
...
- `grill-with-docs` は旧 ADR 更新時に新 ADR 側の関係行を補完せず、明示された関係だけを検証する。

## Direct ADR flows

- `grill-with-docs` は新 ADR をまず `Proposed` として作る。
