...
`vba.md` は逆に `paths` を持つ。VBA の保存形式と識別子制約は `.bas` / `.cls` を触る時にしか意味がなく、他の作業では読む価値がない。ただし条件は拡張子だけでなく `src/` 配下にも掛かっており（正本は `dot_claude/rules/vba.md`）、上に書いた src 前提の穴をそのまま持つ。
...
- `vba.md` の `paths` が `src/` 配下に限定されているのが意図か検討漏れか。`src/` 外や repo 直下に置いた `.bas` / `.cls` では発火しない。
