...
| PowerShell              | OS 別               | 未設定          | 未設定              | BOM 付き UTF-8 / LF | 非 Windows は mise `powershell`、Windows は Scoop `pwsh.exe` を使う。`*.ps1` は Windows PowerShell 5.1 互換のため BOM 付き UTF-8 と LF で扱う |
