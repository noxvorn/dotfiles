...
- **volta**: 2025-11 に end-of-maintenance を公式 announce。現在 unmaintained で、メンテナ推奨の移行先は `mise`。今すぐは動くが、放置すると将来の OS update / Node メジャー更新で壊れるリスク。→ 不採用（メンテ終了）。出典: [volta-cli/volta #2080](https://github.com/volta-cli/volta/issues/2080)。
