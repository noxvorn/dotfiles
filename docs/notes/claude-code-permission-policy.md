...
- `sandbox.excludedCommands` は `git` だけに保つ。`allowedDomains` は HTTP(S) proxy 用の許可なので SSH remote（`git@github.com`）には効かず、sandbox 内の `git push` は `nc: authentication method negotiation failed` で必ず失敗する。Strict sandbox（`allowUnsandboxedCommands: false`）では escape hatch も使えないため、公式が残す唯一の逃げ道である `excludedCommands` を使う。`allowUnsandboxedCommands` を `true` へ戻す方法は、失敗した全 command に sandbox 外再試行を開くので採らない。
- `excludedCommands` に載せた command は sandbox の filesystem / network 制限を受けない。`git` については `permissions.deny` の `Bash(git remote set-url *)` / `Bash(git remote add *)` が残り、扱う対象も基本的に repo 内に閉じるため、この 1 件に限って許容する。増やす時は同じ水準の理由を求める。
