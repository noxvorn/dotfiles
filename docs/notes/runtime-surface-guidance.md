...
- `orchestrate`: コード変更・実装・開発依頼の進行入口。Phase 0 で triage し、micro / standard / full の tier に振り分けて、Phase / Gate、repository maintenance、request folder、agent routing、handoff、ユーザー確認つきで進める。tier により通す Phase / Gate を変え、micro は最小工程に省く。Codex では main セッションが lead として agent 出力を次 agent の入力へ明示的に渡し、agent 間の直接通信を前提にしない。Claude Code では lead が specialist subagent と対応 skill を束ねる
