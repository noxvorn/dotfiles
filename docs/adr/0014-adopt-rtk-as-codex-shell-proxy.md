...
`dot_codex/RTK.md` を deployable artifact として管理し、`dot_codex/AGENTS.md.tmpl` から展開先 home directory に合わせた薄い参照だけを置く。
...
- `rtk` は出力整形 proxy であり、sandbox、approval、破壊的操作、secret handling の境界を変更しない。
- broad な `rtk` allow rule は作らず、承認判断は内側の実コマンド単位で扱う。
