...
`capture-push-knowledge` は push 前の outgoing commit 群を確認する補助 skill であり、Git 操作そのものの user-facing な正式入口ではない。

## Decision

- Git 操作の正式 skill は `git-commit` と `git-push` だけにする。
- `capture-push-knowledge` は `git-push` の push 前補助として扱う。
